...
def single_prediction(predictor, scaler, cv, text_input):
    ...
    y_probs = predictor.predict_proba(X_prediction_scl)
    print("Model classes:", predictor.classes_)


    print("Processed review:", review)
    print("Vector shape:", X_prediction.shape)
    print("Prediction probabilities:", y_probs)

    y_pred_index = y_probs.argmax(axis=1)[0]
    predicted_class = predictor.classes_[y_pred_index]
...
def sentiment_mapping(x):
    if x == "Negative" or x == 0:
        return "Negative"
    elif x == "Positive" or x == 1:
        return "Positive"
    else:
        return str(x)
